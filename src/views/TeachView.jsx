  ...
  return (
    <div class="error-card">
      ...
      <p style="color: var(--text-muted); font-size: 14px;">
        You need leeches — items with at least 5 reviews, a very low success rate ({'<30%'}), due within the next 3 days.
      </p>
      <button
        class="error-option-btn"
...
export default function TeachView({ showPopup, hidePopup, navigate, roteMode }) {
  ...
  const maybeRecordResult = useCallback(() => {
    ...
    // Check if the word is a leech: >0 attempts, success rate <30%
    const data = card.data;
    const isLeech = data.attempts >= 5 && data.successes / data.attempts < 0.30;
