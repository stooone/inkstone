 ...
 *  Copyright 2026 Juhász Péter Károly (stone "at" midway.hu)
 *
...
const createCanvas = (element, handwriting) => {
  ...
  const elWidth = element.offsetWidth || element.clientWidth || kCanvasSize;
  canvas.style.width = canvas.style.height = `${elWidth}px`;
  element.appendChild(canvas);

  const touch_supported = 'ontouchstart' in window;
  const zoom = kCanvasSize / elWidth;
...
const pathToShape = (path, size, color, uncached) => {
  ...
  while (index < tokens.length) {
    ...
    const args = Array.from({length: arity[command] || 0}, () => next());
    if (command === 'Z') {
